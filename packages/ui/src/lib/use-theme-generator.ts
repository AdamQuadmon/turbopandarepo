...
const syncColorPalette = (color: ColorPalette, colorMode: "dark" | "light") => {
	const root = document.querySelector<HTMLHtmlElement>(":root");
	if (root) {
		if (color.value === "neutral") {
			root.style.setProperty(
				...
				token.var(
					colorMode === "light" ? "colors.neutral.950" : "colors.white",
				),
			);
			root.style.setProperty(
			...
			root.style.setProperty(
				"--colors-accent-fg",
				token.var(
					colorMode === "light" ? "colors.white" : "colors.neutral.950",
				),
			);
			root.style.setProperty(
			...
			root.style.setProperty(
				"--colors-accent-default",
				token.var(
					`colors.${color.value}.${colorMode === "light" ? "600" : "300"}`,
				),
			);
			root.style.setProperty(
				"--colors-accent-emphasized",
				token.var(
					`colors.${color.value}.${colorMode === "light" ? "700" : "400"}`,
				),
			);
			root.style.setProperty(
				"--colors-accent-fg",
